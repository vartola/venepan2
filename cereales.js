...
const tabProdBtn = document.getElementById('tab-produccion-btn');
const tabDespBtn = document.getElementById('tab-despacho-btn');
...
if (tabProdBtn && tabDespBtn) {
    tabProdBtn.addEventListener('click', () => {
        tabProdBtn.classList.add('active');
        tabDespBtn.classList.remove('active');
        contenedorProd.style.display = 'block';
        contenedorDesp.style.display = 'none';
    });
    tabDespBtn.addEventListener('click', () => {
        tabDespBtn.classList.add('active');
        tabProdBtn.classList.remove('active');
        contenedorProd.style.display = 'none';
        contenedorDesp.style.display = 'block';
    });
}
    });
});

// ==================== FUNCIONES DE CARGA (PHP/MySQL) ====================
...
async function cargarTablasCerealesPorPeriodo(mes, anio) {
    try {
        ...
        // ----- TABLA DE PRODUCCIÓN: AGRUPADA POR TIPO DE CEREAL -----
        const contenedorProd = document.getElementById('contenedor-produccion');
        ...
        if (!produccion || produccion.length === 0) {
            ...
            // Agrupar por tipo_cereal
            const grupos = {};
            produccion.forEach(item => {
                const tipo = item.tipo_cereal || 'Sin tipo';
            ...
            for (const [tipo, registros] of Object.entries(grupos)) {
                ...
                htmlTotal += `
                    <div class="tabla-producto card" style="background:#e8f5e9; margin-bottom:20px; border:1px solid #a5d6a7;">
                        <div style="display:flex; justify-content:space-between; align-items:center; padding:10px; background:#c8e6c9;">
                            <h3 style="color:#2e7d32;">${tipo} – ${getNombreMes(mes)} ${anio}</h3>
                            <button class="btn-expand" onclick="expandirTablaCereal('tabla-${tipo.replace(/\s/g, '')}')" 
                                style="background:#b33939; color:white; border:none; padding:6px 12px; border-radius:4px; cursor:pointer;">
                                ⛶ Expandir
                            </button>
                        </div>
                        <div style="overflow-x:auto;">
                            <table style="width:100%; border-collapse:collapse; font-size:11px;" id="tabla-${tipo.replace(/\s/g, '')}">
        ...
        // ----- TABLA DE DESPACHO (sin cambios, sigue igual) -----
        const contenedorDesp = document.getElementById('contenedor-despacho');
        if (contenedorDesp) {
            if (!despacho || despacho.length === 0) {
                ...
                let htmlDesp = `
                    <div class="card tabla-producto" style="background:#e8f5e9; border:1px solid #a5d6a7;">
                        <div class="card-header" style="background:#c8e6c9;">
                            <h3 style="color:#2e7d32;"><i class="fas fa-clipboard-check"></i> Despacho de Cereales – ${getNombreMes(mes)} ${anio}</h3>
                        </div>
                        <div style="overflow-x:auto;">
...
window.editarProduccion = async function(id) {
    ...
    try {
        ...
        if (item && !item.error && item.id) {
            // PRIMERO abrir el modal para que el formulario se cree
            abrirModalFormulario('produccion');
            
            // ESPERAR un momento para que el DOM se actualice
            setTimeout(() => {
                // AHORA sí, llenar los campos que ya existen
            ...
            console.error('No se encontró el registro o error:', item);
...
window.editarDespacho = async function(id) {
    console.log('Editando despacho con ID:', id);
...
// ==================== FUNCIONES EXISTENTES (MODALES, CÁLCULOS, ETC) ====================
...
function expandirTablaCereal(idTabla) {
    ...
    if (!modal) {
        // Crear modal si no existe (puedes copiar el mismo modal de panificación)
...
function actualizarNotificacionesCereales(produccion, despacho) {
    ...
    // Rendimiento promedio (de despacho)
    let rendProm = 0;
    ...
    // Unidades producidas (u12+u8+uL de despacho)
    let totalUnidades = 0;
    if (despacho && despacho.length) {
        totalUnidades = despacho.reduce((sum, item) => {
            return sum + (parseInt(item.u12) || 0) + (parseInt(item.u8) || 0) + (parseInt(item.uL) || 0);
        }, 0);
    }
...
async function cargarTablasCerealesPorPeriodo(mes, anio) {
    try {
        ...
        // ----- TABLA DE PRODUCCIÓN (todas las columnas) -----
        const tbodyProd = document.getElementById('historial-produccion-cereales');
        const theadProd = document.getElementById('thead-produccion');
        if (theadProd && tbodyProd) {
            if (!produccion || produccion.length === 0) {
                tbodyProd.innerHTML = `<tr><td colspan="30" style="text-align:center">No hay registros</td></tr>`;
                theadProd.innerHTML = '';
            } else {
                // Cabecera fija (todos los campos importantes)
                theadProd.innerHTML = `
                    <tr>
                        <th>ID</th><th>Fecha</th><th>Tipo</th><th>Medida Base</th><th>Medida Jarabe</th>
                        <th>Base Reproceso</th><th>Jarabe Reproceso</th><th>Total Medidas</th>
                        <th>Sobrante Seco</th><th>Sobrante Jarabe</th><th>Total Mezcla Proc.</th>
                        <th>Medidas Proc.</th><th>Bobina</th><th>1ra (kg)</th><th>2da (kg)</th>
                        <th>Desecho Mezcla</th><th>Desecho Marmita</th><th>Desecho Empaque</th>
                        <th>Empaque Dañado</th><th>Total Producido</th><th>Merma</th>
                        <th>Total s/Desecho</th><th>Medida Base En</th><th>Medida Jarabe En</th>
                        <th>Unid. Teóricas</th><th>Prod. Teórica (%)</th><th>Acciones</th>
                    </tr>
                `;
                let html = '';
                produccion.forEach(item => {
                    html += `
                        <tr>
                            <td>${item.id}</td>
                            <td>${item.fecha || ''}</td>
                            <td>${item.tipo_cereal || ''}</td>
                            <td>${parseFloat(item.medida_base || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.medida_jarabe || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.base_reproceso || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.jarabe_reproceso || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.total_medidas || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.sobrante_seca || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.sobrante_jarabe || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.total_mezcla_procesada || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.medidas_procesadas || 0).toFixed(2)}</td>
                            <td>${item.bobina || ''}</td>
                            <td>${parseFloat(item.primera || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.segunda || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.desecho_mezcla || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.desecho_marmita || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.desecho_empaque || 0).toFixed(2)}</td>
                            <td>${item.empaque_danado || 0}</td>
                            <td>${parseFloat(item.total_producido || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.merma || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.total_sin_desecho || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.medida_base_en || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.medida_jarabe_en || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.uni_teorica || 0).toFixed(2)}</td>
                            <td>${parseFloat(item.prod_teorica || 0).toFixed(2)}%</td>
                            <td>
                                <button onclick="editarProduccion(${item.id})" style="color:#3498db; border:none;"><i class="fas fa-edit"></i></button>
                                <button onclick="eliminarProduccion(${item.id})" style="color:red; border:none;"><i class="fas fa-trash"></i></button>
                            </td>
                        </tr>
                    `;
                });
        ...
        // ----- TABLA DE DESPACHO (todas las columnas) -----
        const tbodyDesp = document.getElementById('historial-despacho-cereales');
        const theadDesp = document.getElementById('thead-despacho');
        if (theadDesp && tbodyDesp) {
            if (!despacho || despacho.length === 0) {
                tbodyDesp.innerHTML = `<tr><td colspan="20" style="text-align:center">No hay registros</td></tr>`;
                theadDesp.innerHTML = '';
            } else {
                theadDesp.innerHTML = `
                    <tr>
